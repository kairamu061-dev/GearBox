...
| 3 | 戦車回収 | 戦車の ScrapCollector コライダーと接触するとバトル内仮保持スクラップに加算して消滅する |
...
4. `BattleSessionData.AddPendingScrap(1)` を呼び出してバトル内仮保持スクラップに加算し、プールへ返却する
   （RunManager への反映はリザルト画面で行う）
